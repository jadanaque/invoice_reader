  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      fileInput(inputId = "zvm03_csv", 
                label = "ZVM03 (SAP) en formato 'CSV'",
                accept = c(
                  "text/csv",
      ...
      actionButton(inputId = "process_all",
                   label = "PROCESAR ARCHIVOS INGRESADOS"),
      
      br(), br(),
      
      h4("Elaborado por DM Perú", br(),
         img(src = "Logo_blackgold_CMJN.jpg", height = "80px"))
...
server <- function(input, output) {
  ...
  # ZVM03
  zvm03 <- reactive({
    read.csv(input$zvm03_csv$datapath, colClasses = "character", stringsAsFactors = FALSE, fileEncoding="latin1") %>%
      ...
      select(matproveedor, ean13, textobrevematerial, descregistrosanitario, cpe, comentario, codregistrosanitario,
             feinicioregistro, fefinalregistro, signature, paisfabricante,
             proveedor, cantidadentrega, pesobruto) %>%
      mutate(cantidadentrega = parse_number(cantidadentrega) %>% as.integer(),
             pesobruto = parse_number(pesobruto)) %>%
      as_tibble()
  })
  ...
  consolidado <- eventReactive(input$process_all, {
    ...
    #### Add part IV - cruce
      bind_rows(burgos(),
                ...
                lusaskinceuticals(),
                lusalorealparis()) %>%
        left_join(zvm03(), by = c("product" = "matproveedor")) %>%
        distinct(filename, pos_fact, .keep_all = TRUE) %>%
        mutate(peso_bruto_total = unids * pesobruto)
    }
  })
